...
const upsertUser = async (req, res) => {
    try {
        let data;
        if (req.body.id) data = await models.User.findByPk(req.body.id);
        if (data) {
            updateHistory = utils.getUpdateHistory(req.body, data['dataValues']);
            if (!updateHistory) return res.status(STATUS_CODES.BAD_REQUEST).json(defaultResponses.BadRequest);
